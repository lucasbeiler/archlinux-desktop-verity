...
pacstrap -c -K $WORKDIR \
  ...
  jq yq rclone patchutils helix helm kubectl nvim code checksec docker docker-compose age sops fluxcd kustomize opentofu pulumi azure-cli aws-cli spotify-player gurk \
  sway kanshi brightnessctl xdg-desktop-portal-wlr waybar mako waybar swayidle swaylock swaybg fuzzel yazi adobe-source-code-pro-fonts ttf-jetbrains-mono otf-font-awesome
