...
# grep -q "/run/systemd/home/\*\* rwk," "$p" || sed -i '/@{att}@{run}\/systemd\/inhibit\/@{int}\.ref rw,/a \ \ @{run}/systemd/home/@{word}.dont-suspend rwk,' "$p"
# grep -q "/run/systemd/home/\*\* rwk," "$p" || sed -i '/@{att}@{run}\/systemd\/inhibit\/@{int}\.ref rw,/a \ \ @{att}@{run}/systemd/home/@{word}.dont-suspend rwk,' "$p"
...
# EOF
# find /tmp/apparmor.d-out > "${WORKDIR}/.apparmor.d-out.debug.txt"
