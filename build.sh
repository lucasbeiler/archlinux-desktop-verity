...
pacstrap -P -c -K $WORKDIR \
  ...
  mkinitcpio opendoas openssh pamixer fastfetch git unzip zip unrar pipewire-jack power-profiles-daemon python-gobject sof-firmware wireplumber pipewire-pulse pavucontrol mtools dosfstools \
  ...
  jq yq patchutils firecracker helm kubectl nvim lapce helix checksec docker docker-compose age sops fluxcd kustomize opentofu aws-cli spotify-player gurk \
  sway kanshi brightnessctl xdg-desktop-portal-wlr mako swayidle swaylock swaybg ttf-jetbrains-mono arch-repro-status rclone #wayfire waybar

# Build and install roddhjav's apparmor.d profiles.
...
sed -i '/@{att}@{run}\/systemd\/inhibit\/@{int}\.ref rw,/a \ \ @{att}@{run}/systemd/home/@{user}.dont-suspend rw,' "$p"
sed -i '/abi <abi\/5.0>,/a \ \ /etc/tunables.conf r,' apparmor.d/abstractions/glibc
rm -rf apparmor.d/groups/filesystem/mtools apparmor.d/groups/ssh apparmor.d/profiles-g-l/git apparmor.d/groups/procps # Delete policies related to mtools, ssh, git and procps (for now...)
