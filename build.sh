...
pacstrap -c -K $WORKDIR \
  ...
  jq yq patchutils helix helm kubectl nvim code checksec docker docker-compose age sops fluxcd kustomize opentofu pulumi azure-cli aws-cli spotify-player \
...
# Install the latest non-GUI release of SimpleX Chat.
curl -fsSL https://github.com/simplex-chat/simplex-chat/releases/latest/download/simplex-chat-ubuntu-24_04-x86_64 -o "${WORKDIR}/usr/local/bin/simplex-chat"
chmod +x "${WORKDIR}/usr/local/bin/simplex-chat"
curl -fsSL https://github.com/simplex-chat/simplex-chat/releases/latest/download/_sha256sums | grep -qi $(sha256sum "${WORKDIR}/usr/local/bin/simplex-chat" | cut -d' ' -f1) || rm -f ${WORKDIR}/usr/local/bin/simplex-chat
...
rm -rf /tmp/boot_artifacts /hardenedos/bootloader.efi
