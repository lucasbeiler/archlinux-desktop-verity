...
pacstrap -P -K $WORKDIR \
  ...
  bubblewrap-suid nmap flameshot slurp grim xdg-desktop-portal alacritty libnotify vulkan-validation-layers vulkan-icd-loader vulkan-headers vulkan-tools \
  jq yq patchutils helix helm kubectl nvim code checksec docker docker-compose age sops fluxcd kustomize opentofu aws-cli spotify-player gurk \
  sway kanshi brightnessctl xdg-desktop-portal-wlr mako swayidle swaylock swaybg ttf-jetbrains-mono arch-repro-status

# Build roddhjav's apparmor-profiles.
pacman -Sy --noconfirm just go git apparmor
id -u builder &>/dev/null || useradd -m builder
echo "builder ALL=(ALL) NOPASSWD: /usr/bin/pacman" > /etc/sudoers.d/builder
su builder -s /bin/bash <<'EOF'
set -euo pipefail
APPARMOR_D_AUR_COMMIT=42a4aa2a2032ac8914e6c5236a1a174a0c1d8a0c      # Update this to the commit hash you last reviewed at https://aur.archlinux.org/cgit/aur.git/log/?h=apparmor.d-git
APPARMOR_D_UPSTREAM_COMMIT=6b62f22227d3068f864c05b99cacaa3c979e5317 # Update this to the commit hash you last reviewed at https://github.com/roddhjav/apparmor.d
cd ~
git clone https://aur.archlinux.org/apparmor.d-git.git
cd apparmor.d-git
git checkout --quiet "${APPARMOR_D_AUR_COMMIT}"
sed -i 's/just complain/just enforce/g' PKGBUILD
sed -i -E "s|(roddhjav/apparmor\.d\.git)(#[^\"'[:space:]]*)?|\1#commit=${APPARMOR_D_UPSTREAM_COMMIT}|" PKGBUILD
makepkg -s
EOF
APPARMOR_D_PKG=$(find /home/builder/apparmor.d-git -maxdepth 1 -name '*.pkg.tar.zst' -print -quit)
[ -n "$APPARMOR_D_PKG" ] || { echo "makepkg did not produce a package file"; exit 1; }
cp "$APPARMOR_D_PKG" ${WORKDIR}/apparmor.d.pkg.tar.zst

# Install the locally built apparmor.d-git package (with profiles set to enforce mode).
arch-chroot $WORKDIR /bin/bash -c 'pacman -U --noconfirm /apparmor.d.pkg.tar.zst && rm -f /apparmor.d.pkg.tar.zst'
...
# Save information about how much of the packages we've installed are really reproducible.
...
systemctl enable apparmor iptables wpa_supplicant-nl80211@wlo1 dnscrypt-proxy ntpd-rs systemd-networkd systemd-homed mydenyusb updater.timer
