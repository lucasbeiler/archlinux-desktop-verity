...
pacstrap -c -K $WORKDIR \
  ...
  jq yq patchutils helix helm kubectl nvim code checksec age sops fluxcd kustomize opentofu pulumi azure-cli aws-cli gurk spotify-player \
...
systemctl enable apparmor iptables wpa_supplicant-nl80211@wlo1 dnscrypt-proxy ntpd-rs systemd-networkd systemd-homed mydenyusb da-lockout-clear-tpm
