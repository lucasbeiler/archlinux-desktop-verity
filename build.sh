...
curl -L https://github.com/simplex-chat/simplex-chat/releases/latest/download/simplex-chat-ubuntu-24_04-x86_64 -o "${WORKDIR}/usr/local/bin/simplex-chat"
chmod +x "${WORKDIR}/usr/local/bin/simplex-chat"
curl -L https://github.com/simplex-chat/simplex-chat/releases/latest | grep -qi $(sha256sum "${WORKDIR}/usr/local/bin/simplex-chat" | cut -d' ' -f1) || rm -f ${WORKDIR}/usr/local/bin/simplex-chat
