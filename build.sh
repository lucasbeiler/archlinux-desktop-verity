...
pacstrap -P -c -K $WORKDIR \
  ...
  wayfire code waybar kanshi brightnessctl xdg-desktop-portal-wlr xdg-desktop-portal-lxqt mako swayidle swaylock ttf-jetbrains-mono arch-repro-status rclone rsync
...
set -euo pipefail
# Declare variables.
APPARMOR_D_UPSTREAM_COMMIT=35adcf5524a2189e60b34c43c26071615bb48c83 # Update this to the commit hash last reviewed at https://github.com/roddhjav/apparmor.d
H_MALLOC_UPSTREAM_COMMIT=903ed6431ebce1be2a9aa8f625f69e25322537a7   # Update this to the commit hash last reviewed at https://github.com/GrapheneOS/hardened_malloc
...
rm -rf apparmor.d/groups/filesystem/mtools apparmor.d/groups/ssh apparmor.d/profiles-g-l/git apparmor.d/groups/procps # Delete policies related to mtools, ssh, git, procps, etc (at least for now...)
...
just prebuild
just destdir="/tmp/apparmor.d-out" install-base
just destdir="/tmp/apparmor.d-out" install-profiles
...
# For now, let's install my hardened Chromium build this way. Just testing.
# _pkg="chromium-152.0.7977.75-1-x86_64.pkg.tar.zst"
# _url="https://download.opensuse.org/repositories/home:/lucasbeiler/Arch/x86_64/$_pkg"
# _expected_sha256="cc0cafb8761870772d757d3b40ad533ab6e27439178ea4ff626b50018d77f478"
# arch-chroot $WORKDIR /bin/bash -c "
#   curl -LO '$_url' &&
#   echo '$_expected_sha256  $_pkg' | sha256sum -c - &&
#   pacman -U --noconfirm '$_pkg' &&
#   rm -f '$_pkg'
# "
...
systemctl enable apparmor iptables wpa_supplicant-nl80211@wlo1 dnscrypt-proxy ntpd-rs systemd-networkd systemd-homed mydenyusb updater.timer firecracker-net-setup.service aa-early-log.service
