...
pacstrap -P -c -K $WORKDIR \
  ...
  bubblewrap-suid nmap wl-clipboard slurp grim xdg-desktop-portal alacritty libnotify jq yq patchutils firecracker helix checksec spotify-player gurk \
...
sed "s/@ROOTHASH@/${VERITY_HASH}/" ${WORKDIR}/etc/ipe/hardened.policy > /tmp/hardened.policy
rm -f ${WORKDIR}/etc/ipe/hardened.policy

openssl smime -sign \
  -in /tmp/hardened.policy \
  -signer /tmp/sbsign/keys/db/db.pem \
  -inkey /tmp/sbsign/keys/db/db.key \
  -noattr -nodetach -nosmimecap \
  -outform der \
  -out ${WORKDIR}/etc/ipe/hardened.policy.p7b

rm -f /tmp/hardened.policy

# Rebuild initramfs now that the signed policy exists inside $WORKDIR,
cp /tmp/boot_artifacts/* ${WORKDIR}/boot/
rm -f ${WORKDIR}/boot/initramfs-linux-hardened.img
...
# TODO: Remove
cp ${WORKDIR}/etc/ipe/hardened.policy.p7b /hardenedos/
