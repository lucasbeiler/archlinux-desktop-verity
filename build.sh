...
pacstrap -P -c -K $WORKDIR \
  ...
  sway river-classic wayfire wf-shell code waybar kanshi brightnessctl xdg-desktop-portal-wlr mako swayidle swaylock swaybg ttf-jetbrains-mono arch-repro-status rclone rsync
...
cp /tmp/hardened_malloc/out-light/libhardened_malloc-light.so "${WORKDIR}/usr/lib/"
