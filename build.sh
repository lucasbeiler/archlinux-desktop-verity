...
pacstrap -c -K $WORKDIR \
  base ${BASE_DEVEL_WITHOUT_SUDO} wpa_supplicant eza chromium flatpak gptfdisk linux-hardened linux-firmware-intel intel-ucode iptables less ntpd-rs dnscrypt-proxy apparmor tpm2-tss tpm2-tools erofs-utils \
  ...
  jq yq patchutils helix helm kubectl nvim code checksec docker docker-compose age sops fluxcd kustomize opentofu pulumi azure-cli aws-cli gurk spotify-player \
...
# TODO: Populate /data/hosts (first boot).
ln -sf /data/hosts /etc/hosts
...
echo '/data/var/lib/flatpak      /var/lib/flatpak          none   bind,noexec,nosuid,nodev,x-systemd.requires-mounts-for=/data,x-systemd.requires-mounts-for=/var          0  0' >> /etc/fstab
