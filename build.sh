...
pacstrap -c -K $WORKDIR \
  ...
  jq yq patchutils helix code helm kubectl checksec docker docker-compose age sops spotify-player gurk \
  sway kanshi brightnessctl xdg-desktop-portal-wlr mako swayidle swaylock swaybg fuzzel ttf-jetbrains-mono \
  qemu-desktop libvirt virt-manager edk2-ovmf # TODO: Remove this later (maybe readd code/nvim/yazi/tmux).
...
# TODO: Remove this.
echo '/data/var/lib/libvirt        /var/lib/libvirt         none   bind,x-systemd.requires-mounts-for=/data,x-systemd.requires-mounts-for=/var                             0  0' >> /etc/fstab
