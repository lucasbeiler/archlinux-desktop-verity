...
set -euo pipefail
APPARMOR_D_UPSTREAM_COMMIT=6b62f22227d3068f864c05b99cacaa3c979e5317 # Update this to the commit hash last reviewed at https://github.com/roddhjav/apparmor.d
...
just complain # enforce

just destdir="/tmp/apparmor.d-out" install
EOF
find /tmp/apparmor.d-out -type f > "${WORKDIR}/.apparmor.d-out.debug.txt"
...
echo '/data/etc/apparmor/earlypolicy/   /etc/apparmor/earlypolicy/      none   bind,noexec,nosuid,nodev,x-systemd.requires-mounts-for=/data,x-systemd.requires-mounts-for=/var          0  0' >> /etc/fstab
