...
pacstrap -c -K $WORKDIR \
  base base-devel wpa_supplicant eza gptfdisk linux-hardened linux-firmware-intel intel-ucode iptables less ntpd-rs dnscrypt-proxy apparmor tpm2-tss tpm2-tools erofs-utils \
...
pacstrap -C /tmp/pacman-custom.conf -c -K $WORKDIR extra-testing/chromium
...
rm -f $OUTPUT
mkdir /tmp/boot_artifacts && mv ${WORKDIR}/boot/* /tmp/boot_artifacts/
...
sbsign --key /tmp/sbsign/keys/db/db.key \
       --cert /tmp/sbsign/keys/db/db.pem \
       --output ${WORKDIR}/boot/uki-${OS_BUILD_TAG}-signed.efi /tmp/boot_artifacts/uki.efi

rm -rf /tmp/boot_artifacts
